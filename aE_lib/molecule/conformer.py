...
class conformer(object):
	...
	def data_from_file(self, file, type='g09'):
		if type == 'nxyz':
			self.id, self.types, self.xyz, self.dist, self.shift, self.coupling1b, self.coupling2b, self.coupling3b = nxyz_to_nmrdata(file)

		elif type == 'g09':
			self.id, self.types, self.xyz, self.dist, self.shift, self.coupling1b, self.coupling2b, self.coupling3b = g09_to_nmrdata(file)

		elif type == 'opt_g09':
			self.id, self.types, self.xyz, self.dist, self.energy = opt_g09_to_nmrdata(file)

		elif type == 'sygcml':
			self.id, self.types, self.xyz, self.dist, self.shift, self.coupling1b, self.coupling2b, self.coupling3b = sygcml_to_nmrdata(file)

		elif type == 'jxyz':
			self.id, self.types, self.xyz, self.dist, self.shift, self.coupling1b, self.coupling2b, self.coupling3b = jxyz_to_nmrdata(file)

		elif type == 'syg_jxyz':
			self.id, self.types, self.xyz, self.dist, self.shift, self.coupling1b, self.coupling2b, self.coupling3b = syg_jxyz_to_nmrdata(file)

		elif type == 'pdb':
			self.id, self.types, self.xyz, self.dist, self.shift, self.coupling1b, self.coupling2b, self.coupling3b = pdb_to_nmrdummy(file)

		elif type == 'xyz':
			self.id, self.types, self.xyz, self.dist, self.shift, self.coupling1b, self.coupling2b, self.coupling3b = xyz_to_nmrdummy(file)
	...
	def generate_opt_com(self, path='', charge=0, multiplicity=1, memory=26, processors=8,
		...
		self.opt_com = make_optcom(str(self.id), self.xyz, self.types, directory=path,
	...
	def generate_nmr_com(self, path='', charge=0, multiplicity=1, memory=26, processors=8,
		...
		self.nmr_com = make_nmrcom(str(self.id), self.xyz, self.types, directory=path,
							charge=charge, multiplicity=multiplicity, memory=memory, processors=processors,
